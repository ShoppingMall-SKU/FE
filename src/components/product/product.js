...
export const Product = ({ product, convertPrice }) => {
    ...
    return (
           product && (
            <div
                className="container h-56 shadow-xl rounded-2xl p-2 lg:h-5/6 lg:mb-32 md:p-5 max-w-48 md:max-w-xs lg:max-w-screen-sm lg:max-h-screen pl-2 pr-2 lg:pr-4 lg:pl-4 justify-center flex flex-col mb-8">
                <Link to={`/product/${id}`}>
                ...
                <div className="mb-1.5 md:mb-3">
                        <span className="text-xs font-thin md:text-l leading-22 text-gray-600" style={{fontFamily: 'sb'}}>{brand}</span>
                    { stock > 10 ?
                        (<span></span>) :
                ...
                <div className="mb-0.5 md:mb-3">
                    {name.length > 9 ?
                        (<span className="text-sm md:text-3xl leading-22 text-black" style={{fontFamily: 'sb'}}> {name.slice(0, 8)} ...</span>) :
                        (<span className="text-sm md:text-3xl leading-22 text-black" style={{fontFamily: 'sb'}}>{name}</span>)
                    }
                </div>
                ...
                <div>
                    {sale > 0 ? (
                            <>
                                <div>
                                    <span className="text-xs md:text-m font-bold leading-30 text-red-600 line-through">{convertPrice(price)} 원</span>
                                    <span className="text-xs md:text-m font-bold leading-30 text-red-600">  -{sale}%</span>
                                </div>
